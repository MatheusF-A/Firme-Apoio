...
if (!isset($_GET['id']) || empty($_GET['id'])) {
    header("Location: ../paginas/exercicios.php?status=erro&msg=ID do exercicio nao fornecido");
    exit();
}
...
// ----> FIM DA MODIFICAÇÃO <----
    ...
    if ($stmt->rowCount() > 0) {
        ...
        throw new Exception("Nenhum exercicio encontrado ou permissao negada.");
    }
    exit();

} catch (Exception $e) {
    $erroMsg = urlencode("Erro ao atualizar exercicio: " . $e->getMessage());
