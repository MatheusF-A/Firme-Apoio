...
$exercicioID = $_POST['exercicioID'] ?? 0; // ID do exercício que está sendo editado
$nome = $_POST['nome'] ?? '';
$detalhes = $_POST['detalhes'] ?? '';
$frequenciaID = $_POST['frequenciaID'] ?? null;

// Validação
if (empty($nome) || empty($frequenciaID) || empty($exercicioID)) {
    send_json_response('error', 'Erro: Nome, Frequência e ID do Exercício são obrigatórios.');
}
...
try {
    ...
    $sql = "UPDATE exercicios 
            SET nome = :nome, 
                ...
                {$sql_imagem_part}
            WHERE exercicioID = :exercicioID AND usuarioID = :usuarioID";
    ...
    send_json_response('success', 'Exercício atualizado com sucesso!');

} catch (Exception $e) {
    send_json_response('error', 'Erro ao atualizar o exercício: ' . $e->getMessage());
}
?>